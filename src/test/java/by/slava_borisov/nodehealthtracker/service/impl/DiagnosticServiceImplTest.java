...
class DiagnosticServiceImplTest {
    ...
    void diagnose_dnsFailure_returnsDnsLayer() {
        ...
        assertEquals(Messages.DNS_RESOLUTION_FAILED_RECOMMENDATION, result.recommendation());
    }
    ...
    void diagnose_pingFailure_returnsNetworkLayer() {
        ...
        assertEquals(Messages.HOST_UNREACHABLE_RECOMMENDATION, result.recommendation());
    }
    ...
    void diagnose_tcpFailure_returnsPortLayer() {
        ...
        assertEquals(Messages.TCP_CONNECTION_FAILED_RECOMMENDATION, result.recommendation());
    }
    ...
    void diagnose_sslFailure_returnsSslLayer() {
        ...
        assertEquals(Messages.SSL_VALIDATION_FAILED_RECOMMENDATION, result.recommendation());
    }

    @Test
    @DisplayName("Диагностика - сбой heartbeat")
    void diagnose_heartbeatFailure_returnsHeartbeatLayer() {
        DiagnosticResult result = diagnosticService.diagnose(
                true,
                true,
                true,
        ...
        assertEquals(FailureLayer.HEARTBEAT, result.failureLayer());
        assertEquals(Messages.HEARTBEAT_FAILED, result.diagnosticMessage());
        assertEquals(Messages.HEARTBEAT_FAILED_RECOMMENDATION, result.recommendation());
    }
    ...
    void diagnose_http5xxError_returnsApplicationLayer() {
        ...
        assertEquals(Messages.HTTP_SERVER_ERROR_RECOMMENDATION, result.recommendation());
    }
    ...
    void diagnose_http4xxError_returnsApplicationLayer() {
        ...
        assertEquals(Messages.HTTP_CLIENT_ERROR_RECOMMENDATION, result.recommendation());
    }

    @Test
    @DisplayName("Диагностика - медленный ответ (Performance)")
    void diagnose_slowResponse_returnsPerformanceLayer() {
        ...
        assertEquals(Messages.SLOW_RESPONSE_RECOMMENDATION, result.recommendation());
    }

    @Test
    @DisplayName("Диагностика - сервис здоров (все проверки пройдены)")
    void diagnose_healthyService_returnsUnknownLayer() {
        ...
        assertEquals(Messages.NO_CRITICAL_PROBLEM_DETECTED, result.diagnosticMessage());
